    ...
    <div class="col-md-6 col-12">
        <div class="card">
            <div class="card-header">
                <span class="mb-0 fs-5"><i class="bi bi-pencil-square me-2"></i>แก้ไขบัญชีผู้ใช้</span>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('users.update', $user->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        <label for="password" class="form-label fw-medium">รหัสผ่านสำหรับเข้าใช้งาน (เว้นว่างหากไม่ต้องการเปลี่ยน)</label>
                        <input type="password" class="form-control @error('password') is-invalid @enderror" 
                               id="password" name="password" 
                               placeholder="กรอกเฉพาะเมื่อต้องการตั้งรหัสผ่านใหม่...">
                        @error('password')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-select @error('role') is-invalid @enderror" id="role" name="role" required>
                            ...
                            <option value="staff" {{ old('role', $user->role) === 'staff' ? 'selected' : '' }}>เจ้าหน้าที่ปฏิบัติงาน (Staff)</option>
                            <option value="viewer" {{ old('role', $user->role) === 'viewer' ? 'selected' : '' }}>ผู้ดูรายงาน (Viewer)</option>
                        </select>
                        @error('role')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
